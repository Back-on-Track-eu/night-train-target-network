...
Copy the example file and fill in your credentials path:
```bash
cp .devcontainer/env.example .devcontainer/.env
```

Then open `.devcontainer/.env` and set the path to your Google service account JSON file:
```
...
Open the repo in VS Code. When prompted, click **Reopen in Container**. If the prompt does not appear, open the command palette (`Ctrl+Shift+P`) and run **Dev Containers: Reopen in Container**.

VS Code will build the images, start the containers, and run the post-create script automatically. The terminal will show `Data loaded.` when the backend is ready.
...
When you see the following in the terminal the backend is ready:
```json
...
**Docker Compose fails with `Set GOOGLE_APPLICATION_CREDENTIALS in .env`**  
The `.env` file is missing or the variable is empty. Create `.devcontainer/.env` from `env.example` and set the path.
