...
def _src(row, source_id_field: str, sources: dict[int, ParamsSource]) -> ParamsSource | None:
...
class DBDataLoader:
    ...
    def build_composition(self, comp_id: str) -> tuple[Composition, ParamVersions]:
        ...
        for cr in class_rows:
            ...
            if sc is None:
                logger.warning(
                    ...
                    "at position %d for composition '%s'.", cr["service_class_id"], pos, comp_id
                )
                continue
            classes_by_position.setdefault(pos, {})[cr["service_class_id"]] = CoachClassAssignment(
        ...
        operator = self._load_operator(comp_row["composition_type_operator_id"], sources)
        ...
        comp_type = CompositionType(
            ...
            energy_factor_terrain  = _f(comp_row["composition_type_energy_factor_terrain"]),
            min_boarding_time_min  = _interval_to_min(comp_row["composition_type_min_boarding_time"]),
            min_alighting_time_min = _interval_to_min(comp_row["composition_type_min_alighting_time"]),
        ...
        comp_descriptions = self._load_column_comments("input_params", "composition_types")
    ...
    def build_all_compositions(self) -> tuple[dict[str, Composition], ParamVersions]:
            ...
            try:
                ...
                if ref_row:
                    ...
                    try:
                        ...
                        comp.indicative = compute_indicative_figures(comp, ref, tracks, stop_infra)
                    ...
                    logger.warning(
                        "No composition_references row for '%s' — indicative figures unavailable.", comp_id
                    )
                    comp.indicative = None
        ...
        logger.info("Built %d compositions (%d with indicative figures).",
                    len(result), sum(1 for c in result.values() if c.indicative))
    ...
    def build_all_tracks(self) -> tuple[TrackInfraCollection, ParamVersions]:
        ...
        if default_row is None:
            raise ValueError("No track infrastructure defaults found — cannot resolve missing values.")
        ...
        default = DefaultTrackInfra(
            ...
            min_boarding_time_min  = _interval_to_min(default_row["track_min_boarding_time"]),
            min_boarding_src       = _src(default_row, "track_min_boarding_src", sources),
            min_alighting_time_min = _interval_to_min(default_row["track_min_alighting_time"]),
        ...
        track_descriptions = self._load_column_comments("input_params", "track_infrastructures")
        ...
        for row in rows:
            ...
            try:
                ...
                track_fields  = {
                    ...
                    "energy_price_eur_kwh": (track.energy_price_eur_kwh,   track.energy_price_src),
                    ...
                    "min_boarding_time_min":(track.min_boarding_time_min,  track.min_boarding_src),
                    "min_alighting_time_min":(track.min_alighting_time_min, track.min_alighting_src),
    ...
    def _row_to_track(
        ...
        tac_val,       tac_def       = resolve("tac_eur_train_km",       _f_or_none(row["track_tac_eur_train_km"]),                default.tac_eur_train_km)
        parking_val,   parking_def   = resolve("parking_eur_day",        _f_or_none(row["track_parking_eur_day"]),                 default.parking_eur_day)
        energy_val,    energy_def    = resolve("energy_price_eur_kwh",   _f_or_none(row["track_energy_price_eur_kwh"]),            default.energy_price_eur_kwh)
        terrain_val,   terrain_def   = resolve("terrain_score",          _f_or_none(row["track_terrain_score"]),                   default.terrain_score)
        terr_cat_val,  terr_cat_def  = resolve("terrain_category",       row.get("track_terrain_category"),                       default.terrain_category)
        hsr_val,       hsr_def       = resolve("hsr_allowed",            _b_or_none(row["track_hsr_allowed"]),                    default.hsr_allowed)
        board_val,     board_def     = resolve("min_boarding_time_min",  _interval_to_min_or_none(row["track_min_boarding_time"]),  default.min_boarding_time_min)
        alight_val,    alight_def    = resolve("min_alighting_time_min", _interval_to_min_or_none(row["track_min_alighting_time"]), default.min_alighting_time_min)
        buffer_val,    buffer_def    = resolve("buffer_quota_per",       _f_or_none(row["track_buffer_quota_per"]),                default.buffer_quota_per)
        ...
        return TrackInfrastructure(
            ...
            tac_src                = field_src("track_tac_src")          or (default.tac_src          if tac_def       else None),
            parking_eur_day        = parking_val,
            parking_src            = field_src("track_parking_src")      or (default.parking_src      if parking_def   else None),
            energy_price_eur_kwh   = energy_val,
            energy_price_src       = field_src("track_energy_price_src") or (default.energy_price_src if energy_def    else None),
            terrain_score          = terrain_val,
            terrain_category       = terr_cat_val,
            terrain_src            = field_src("track_terrain_src")      or (default.terrain_src      if terrain_def   else None),
            hsr_allowed            = hsr_val,
            hsr_src                = field_src("track_hsr_src")          or (default.hsr_src          if hsr_def       else None),
            min_boarding_time_min  = board_val,
            min_boarding_src       = field_src("track_min_boarding_src") or (default.min_boarding_src if board_def     else None),
            min_alighting_time_min = alight_val,
            min_alighting_src      = field_src("track_min_alighting_src") or (default.min_alighting_src if alight_def  else None),
            buffer_quota_per       = buffer_val,
            buffer_src             = field_src("track_buffer_src")       or (default.buffer_src       if buffer_def    else None),
        )
    ...
    def build_all_stops(self) -> tuple[StopInfraCollection, ParamVersions]:
        ...
        if global_default is None:
            raise ValueError("No global stop default found — cannot resolve missing stop charges.")

        stop_descriptions = self._load_column_comments("input_params", "stop_infrastructures")
    ...
    def _row_to_stop(
        ...
        charge, charge_is_default = resolve("stop_charge_eur", _f_or_none(row["stop_charge_eur"]), default.stop_charge_eur)
