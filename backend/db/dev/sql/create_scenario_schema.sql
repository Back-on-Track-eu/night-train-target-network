...
-- versioned input_params table. Every read of input_params data goes
-- through a scenario — there is no other notion of "current" left in
-- the database (see create_input_params_schema.sql: the eight
-- versioned parameter tables carry no is_current flag of their own).
--
-- Versioning contract for the eight tables referenced below
-- ------------------------------------------------------------
...
-- that e.g. "composition_types WHERE composition_type_version = 7"
...
-- new version. Given table sizes here (dozens of compositions, ~40
-- countries, a few hundred stops) this is negligible.
--
...
-- eight parameter tables). scenario_key is the stable identifier for
...
CREATE TABLE scenario.scenarios (
    ...
    operators_version                       INTEGER NOT NULL,
    coach_types_version                     INTEGER NOT NULL,
    composition_types_version               INTEGER NOT NULL,
    composition_references_version          INTEGER NOT NULL,
...
COMMENT ON TABLE  scenario.scenarios IS 'Container pinning one version of each versioned input_params table. Exactly one row has is_current_base = TRUE (the live default); exactly one row per scenario_key has is_current_scenario = TRUE (the head of that what-if lineage). All eight *_version columns are per-table full-snapshot version numbers, resolved by exact match, and are NOT NULL — a scenario is always a complete, self-contained pin, never a partial diff.';
...
COMMENT ON COLUMN scenario.scenarios.operators_version                     IS 'Pinned input_params.operators version (full-table snapshot).';
COMMENT ON COLUMN scenario.scenarios.coach_types_version                   IS 'Pinned input_params.coach_types version (full-table snapshot).';
COMMENT ON COLUMN scenario.scenarios.composition_types_version             IS 'Pinned input_params.composition_types version (full-table snapshot).';
COMMENT ON COLUMN scenario.scenarios.composition_references_version        IS 'Pinned input_params.composition_references version (full-table snapshot).';
