...
class Breakdown:
    ...
    def _scale(self, factor: float) -> "Breakdown":
        ...
        return Breakdown(
            ...
            svc_stockings_by_class={
                k: v * factor for k, v in self.svc_stockings_by_class.items()
            },
    ...
    def per_trip_km(self, total_distance_km: float) -> "Breakdown":
        ...
        return (
            self._scale(1.0 / total_distance_km)
    ...
    def per_available_place_km(self, total_available_place_km: float) -> "Breakdown":
        ...
        return (
            self._scale(1.0 / total_available_place_km)
            if total_available_place_km > 0
            else self._scale(0)
        )
    ...
    def per_sold_place_km(self, total_sold_place_km: float) -> "Breakdown":
        ...
        return (
            self._scale(1.0 / total_sold_place_km)
            if total_sold_place_km > 0
            else self._scale(0)
        )
    ...
    def per_available_place_of_class(self, available_places: int) -> "Breakdown":
        ...
        return (
            self._scale(1.0 / available_places)
    ...
    def per_available_place_km_of_class(
        self, available_places: int, distance_km: float
    ) -> "Breakdown":
    ...
    def per_sold_place_km_of_class(
        self, sold_places: int, distance_km: float
    ) -> "Breakdown":
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "per_available_place_of_class": {
                k: v.to_dict() for k, v in self.per_available_place_of_class.items()
            },
            "per_sold_place_of_class": {
                k: v.to_dict() for k, v in self.per_sold_place_of_class.items()
            },
            "per_available_place_km_of_class": {
                k: v.to_dict() for k, v in self.per_available_place_km_of_class.items()
            },
            "per_sold_place_km_of_class": {
                k: v.to_dict() for k, v in self.per_sold_place_km_of_class.items()
            },
        }
...
def _build_matrix(
    ...
    for cls, avail in available_by_class_main.items():
        ...
        per_available_place_of_class[cls] = breakdown.per_available_place_of_class(
            avail
        )
        per_sold_place_of_class[cls] = breakdown.per_sold_place_of_class(sold)
        per_available_place_km_of_class[cls] = (
            breakdown.per_available_place_km_of_class(avail, total_distance_km)
        )
        per_sold_place_km_of_class[cls] = breakdown.per_sold_place_km_of_class(
            sold, total_distance_km
        )
    ...
    return NormalisedMatrix(
        ...
        per_available_place_km=breakdown.per_available_place_km(
            total_available_place_km
        ),
...
def _density_by_class_main(composition: Composition) -> dict[str, float]:
    ...
    return {
        ...
        for main in total_places
        if total_places[main] > 0
    }
...
def _legs_for_od(
    trip: Trip, origin_stop_id: str, dest_stop_id: str
) -> list[CountryLeg]:
...
def _build_trip_breakdown(
    ...
    loco_amort = (
        ...
        if loco_avail_days > 0 and composition.loco_amort_years > 0
        else 0.0
    )
    steps.append(
        CalcStep(
    ...
    coach_amort = (
        composition.purchase_coach_eur
        / (coach_avail_days * composition.coach_amort_years)
        if coach_avail_days > 0 and composition.coach_amort_years > 0
        else 0.0
    )
    steps.append(
        CalcStep(
    ...
    financing = (
        (composition.purchase_loco_eur + composition.purchase_coach_eur)
        * composition.financing_quota_per
        / 365.0
    )
    steps.append(
        CalcStep(
            "financing",
            {
                "purchase_total_eur": composition.purchase_loco_eur
                + composition.purchase_coach_eur,
    ...
    steps.append(
        CalcStep(
            "station_charges", {"n_stops": float(len(trip.stop_times))}, station_charges
        )
    )
...
def _build_od_breakdown(
    ...
    time_share = (
        od_driving_time_h / trip_driving_time_h if trip_driving_time_h > 0 else 0.0
    )
    ...
    trip_density_place_km = sum(
        ...
        * sum(
            cl.distance_km
            for cl in _legs_for_od(trip, trod.origin_stop_id, trod.destination_stop_id)
        )
        for trod in trip_demand.od_pairs
    )
    fixed_share = (
        od_density_place_km / trip_density_place_km
        if trip_density_place_km > 0
        else 0.0
    )
    ...
    steps.append(
        CalcStep(
            "station_charges", {"n_stops": float(len(od_stops))}, od_station_charges
        )
    )
    ...
    steps.append(
        CalcStep(
            "loco_maintenance",
            {"trip_loco_maint": trip_bd.loco_maintenance, "dist_share": dist_share},
            od_loco_maint,
        )
    )
    steps.append(
        CalcStep(
            "coach_maintenance",
            {"trip_coach_maint": trip_bd.coach_maintenance, "dist_share": dist_share},
            od_coach_maint,
        )
    )
    ...
    steps.append(
        CalcStep(
            "driver_cost",
            {"trip_driver": trip_bd.driver, "time_share": time_share},
            od_driver,
        )
    )
    steps.append(
        CalcStep(
            "crew_cost", {"trip_crew": trip_bd.crew, "time_share": time_share}, od_crew
        )
    )
    ...
    steps.append(
        CalcStep(
            "loco_amortisation",
            {"trip_loco_amort": trip_bd.loco_amortisation, "fixed_share": fixed_share},
            od_loco_amort,
        )
    )
    steps.append(
        CalcStep(
            "coach_amortisation",
            {
                "trip_coach_amort": trip_bd.coach_amortisation,
    ...
    steps.append(
        CalcStep(
            "financing",
            {"trip_financing": trip_bd.financing, "fixed_share": fixed_share},
            od_financing,
        )
    )
    steps.append(
        CalcStep(
            "fix_overhead",
            {"trip_fix_overhead": trip_bd.fix_overhead, "fixed_share": fixed_share},
            od_fix_overhead,
        )
    )
    steps.append(
        CalcStep(
            "cleaning",
            {"trip_cleaning": trip_bd.cleaning, "fixed_share": fixed_share},
            od_cleaning,
        )
    )
    steps.append(
        CalcStep(
            "shunting",
            {"trip_shunting": trip_bd.shunting, "fixed_share": fixed_share},
            od_shunting,
        )
    )
    steps.append(
        CalcStep(
            "parking",
            {"trip_parking": trip_bd.parking, "fixed_share": fixed_share},
            od_parking,
        )
    )
    ...
    steps.append(
        CalcStep(
            "svc_stockings_per_class",
            {"svc_eur_place": svc_rate, "places_sold": float(od.places_sold)},
            od_svc,
        )
    )
    steps.append(
        CalcStep(
            "var_overhead",
            {"trip_var_overhead": trip_bd.var_overhead, "rev_share": rev_share},
            od_var_oh,
        )
    )
    steps.append(
        CalcStep(
            "ebit_margin",
            {"trip_ebit_margin": trip_bd.ebit_margin, "rev_share": rev_share},
            od_ebit,
        )
    )
...
def _sum_breakdowns(bds: list[Breakdown], scope: str = "full") -> Breakdown:
    ...
    for bd in bds:
        ...
        for cls, svc in bd.svc_stockings_by_class.items():
            result.svc_stockings_by_class[cls] = (
                result.svc_stockings_by_class.get(cls, 0.0) + svc
            )
                    ...
                    "source": (
                        {
                            ...
                            "source_date": (
                                str(v.source.source_date)
                                if v.source.source_date
                                else None
                            ),
                        }
                        if v.source
...
def compute_indicative_figures(
    ...
    ref_track = TrackInfrastructure(
        ...
        energy_price_eur_kwh=(
            default_track.energy_price_eur_kwh if default_track else 0.18
        ),
    ...
    ref_stop = _SI(
        stop_id="__REF_ORIGIN__",
        stop_name="Reference Origin",
        stop_country_code="__REF__",
        lat=0.0,
        lon=0.0,
        loc_src=None,
        stop_charge_eur=0.0,
        stop_charge_src=None,
    )
    ref_stop2 = _SI(
        stop_id="__REF_DEST__",
        stop_name="Reference Destination",
        stop_country_code="__REF__",
        lat=0.0,
        lon=0.0,
        loc_src=None,
    ...
    ref_stop_times = [
        ...
        StopTime(
            ...
            lon=0.0,
            stop_type="alighting",
            arrival_time_min=driving_min + buffer_min,
    ...
    sold_pax_km = sum(od.places_sold * ref.ref_distance_km for od in od_pairs)
    ...
    subsidy_per_pax_km = (
        (bd.total_cost - bd.total_revenue) / sold_pax_km if sold_pax_km > 0 else 0.0
    )
    ...
    breakeven_load = (
        bd.total_cost
        / bd.total_revenue
        * sum(ref.ref_utilization_by_class.get(cls, 0.0) for cls in places_main)
        / len(places_main)
        if bd.total_revenue > 0 and places_main
        else 0.0
    )
...
def evaluate_route(
    ...
    for trip in all_trips:
        ...
        for od in trip_demand.od_pairs:
            ...
            od_avail_plkm = (
                places_main.get(od.class_main, 0)
                * density_main.get(od.class_main, 1.0)
                * od_dist_km
            )
            od_sold_plkm = (
                od.places_sold * density_main.get(od.class_main, 1.0) * od_dist_km
            )
    ...
    total_sold_place_km = sum(
        ...
        * sum(
        ...
        for trip, td in zip(
            all_trips, [route_demand.get_trip_demand(t.trip_id) for t in all_trips]
        )
    ...
    summary_matrix = _build_matrix(
        ...
        n_trips,
        total_dist_km,
        total_avail_place_km,
        total_sold_place_km,
        places_main,
        dict(total_sold_main),
    )
    ...
    model_versions = (
        first_trip.model_versions if first_trip else ModelVersions(versions={})
    )
    ...
    logger.info(
        ...
        n_trips,
        summary_bd.total_revenue,
        summary_bd.total_cost,
        summary_bd.margin,
        len(od_results),
        len(by_country),
    )
