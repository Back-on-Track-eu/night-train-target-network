...
**`CALC_VERSION = "0.9.15"`**. These numbers get set when the salvaged
...
**0.3.1 Gather.** For every row with a real named file (48 infra + 10
compositions = 58 files), locate the underlying document on disk. Two
...
**0.3.3 Naming convention (decision needed before upload).** The
...
- **Gitignore additions** (compositions active now; infra when its files
  land):

  ```gitignore
  # Calibration outputs — generated by the calib notebooks; re-run the
  # notebooks (01 then 02, top to bottom) to regenerate. Never hand-edit.
  models/compositions/calib/data/
  models/compositions/calib/figures/
  models/compositions/calib/CALIBRATION.md
  models/infrastructure/calib/data/
  models/infrastructure/calib/*/data/
  ```
...
6. `api/helpers/route_serialize.py`: round-trip `countries` and
   ...
   `route/model.py`, per §0.2) + changelog entry.
7. `models/evaluation/calc.py`: replace the flat
   ...
   `SegmentCost` as far as views need. Bump `CALC_VERSION → 0.9.15`
   (in `evaluation/model.py`, per §0.2).
...
| Step | Scope | Size |
|---|---|---|
| 1 | §0 groundwork + §1.1 TAC review | ½–1 day |
| 2 | §0.4 Drive CSV mechanism (`calib_data.py`, both zips, gitignore) + §1.2–1.3 TAC schema + seeding | 1–1.5 days |
| 3 | §1.4–1.6 TAC model integration + tests + docs | 2–3 days |
| 4 | §2 electricity | ½–1 day |
| 5 | §3 shunting/parking | 1 day |
| 6 | §4 route context | ½–1 day |
| 7 | §6 wrap-up + PR | ½ day |

Compositions-calib migration to Drive (its half of §0.4) is decoupled from
the infra work — that domain is already implemented and seeding
correctly, so its zip/env-var switch can happen in step 2 alongside the
infra one, or slip independently without blocking anything downstream.
