...
| 6a | Resolve a CSV of named candidates to OSM ids and print paste-ready step 6 lines (coordinate-first, name-second, step 6's guards applied) — the tool behind the 2026-09 gap closure and the way to add the next batch | ✅ done | `step6a_resolve_candidates.py`, `step6_gap_closure_2026-09.csv`, `step6_expert_review_2026-09.csv` |
...
| 9 | Calibrate station charges from the countries' tariff documents — one CSV per country in `charges/sources/` following `charges/sources/TEMPLATE.md`, joined onto the catalog by step 10 | 🔄 20 countries in `sources/` (981 of 1,214 stops: 739 priced per call, 20 priced per tonne, 222 sourced as not levied); GB, NO, FI and the Balkans open — see `HANDOVER.md` §3 | `charges/01_source_extraction.ipynb`, `charges/02_station_charges.ipynb` |
...
| `step6_expert_review_2026-09.csv` | **Tracked.** Step 6a input for the first expert-review batch (Josua, 2026-09-07): 40 stations a target network needs that no schedule ever had — Balaton and Alpine tourism, Greek and Balkan corridors, ferry and border stations — plus the two deliberately not added. Written against the gap-closure file's column shape. | step 6a |
...
| `data/stop_seed_catalog.csv` | Step 10 output, **the one published file**: the seven seed contract columns first, the charge provenance, a human-readable `provenance` category, `infra_versions`, then the step 7 name/city/language columns and step 8 gauges. `seed.py` validates the 37-column header as an **exact** match (`_STOP_SEED_CSV_COLUMNS`) — step 10 and seed.py move in lockstep. `stop_charge_per_tonne_eur` beside `stop_charge_eur` carries a mass-based tariff (Czechia, `basis per_call_per_tonne`): the rate per tonne of coach mass, with an explicit `0.00` fixed part; the cost model multiplies it by `Composition.total_weight_t` at evaluation time. Step 10 also checks every value against the column widths of `input_params.stop_infrastructures` (`SEED_MAX_LEN`) and refuses to write an over-long one, naming the stop and column — the alternative is a bare `value too long for type character varying(60)` at container start. Where OSM tags a station with several `uic_ref`s, the catalog keeps the first (the column holds one); the full list stays in `step7_enriched_stops.csv`. `infra_versions` is carried but not yet acted on at seed time. | DB seed |
