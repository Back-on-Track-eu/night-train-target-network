...
| 9 | Calibrate station charges from the countries' tariff documents — one CSV per country in `charges/sources/` following `charges/sources/TEMPLATE.md`, joined onto the catalog by step 10 | 🔄 19 countries in `sources/` (961 of 1,214 stops: 739 priced, 222 sourced as not levied); CZ registered but per-tonne, not yet joined; GB, NO, FI and the Balkans open — see `HANDOVER.md` §3 | `charges/01_source_extraction.ipynb`, `charges/02_station_charges.ipynb` |
