...
for comp_id, c in compositions.all().items():
    print(
        f"  {comp_id:15s}  {c.comp_description:30s}  "
...
if nj:
    ...
    print(
        f"  driver_overhead_h:       {nj.driver_overhead_h:.4f}h ({nj.driver_overhead_h*60:.0f} min)"
    )
    print(
        f"  crew_overhead_h:         {nj.crew_overhead_h:.4f}h ({nj.crew_overhead_h*60:.0f} min)"
    )
...
for cc in ["AT", "DE", "FR", "BE", "_default"]:
    ...
    if ip:
        print(
            f"  [{cc:8s}]  tac={ip.tac_eur_train_km:.2f}  "
...
for stop_id in ["Wien Hbf", "Salzburg Hbf", "München Hbf"]:
    ...
    if sp:
        print(
            f"  {sp.stop_id:20s}  {sp.stop_country_code}  "
...
for relation_id, d in demand.all().items():
    print(
        f"  {relation_id:25s}  {d.origin_stop_id:15s} → {d.destination_stop_id:15s}  "
...
for key in [
    "comp_description",
    "comp_purchase_loco_eur",
    "comp_purchase_coach_eur",
    "comp_cleaning_services_eur_day",
    "comp_shunting_eur_day",
]:
