    ...
    def test_total_energy_equals_sum_of_legs(self, route_de_at):
        for trip in route_de_at["trips"]:
            ...
            assert leg_sum == pytest.approx(
                trip["stats"]["total_energy_kwh"], rel=1e-3
            )
            ...
            for seg in trip["path"]["segments"]:
                for cl in seg["country_legs"]:
                    ...
                    if dist_km > 0:
                        ...
                        assert cl["energy_kwh_per_km"] == pytest.approx(expected, rel=1e-2)
...
class TestEnergyByTerrain:
    ...
    @pytest.mark.skip(reason="Terrain effect requires calibrated regression model. Re-enable after energy team calibrates.")
    def test_mountainous_country_has_higher_energy_per_km(self, route_de_ch_at):
        ...
        ch_legs = [
            cl for trip in route_de_ch_at["trips"]
            for seg in trip["path"]["segments"]
...
class TestEnergyByComposition:
    ...
    def test_heavier_composition_uses_more_energy(self, api_base):
        ...
        for comp_id in ("STD-3.1", "STD-13.1"):
            route = _build_route(api_base, STOPS_DE_AT, comp_id=comp_id,
        ...
        assert energies["STD-13.1"] >= energies["STD-3.1"], \
    ...
    def test_same_composition_outbound_return_same_energy(self, route_de_at):
        ...
        outbound_energy = next(
            t["stats"]["total_energy_kwh"]
            for t in route_de_at["trips"] if t["direction_id"] == 0
        )
        return_energy = next(
            t["stats"]["total_energy_kwh"]
            for t in route_de_at["trips"] if t["direction_id"] == 1
        )
...
class TestEnergyModelVersion:
    ...
    def test_energy_version_present_in_trip(self, route_de_at):
        for trip in route_de_at["trips"]:
            ...
            mv_str = str(mv)
            assert "energy" in mv_str.lower() or len(mv) > 0,                 f"model_versions appears empty or missing energy info: {mv}"

    def test_energy_version_is_string(self, route_de_at):
        for trip in route_de_at["trips"]:
            mv = trip["model_versions"]
            # Accept either {'energy_calc': '1.0.0'} or serialised form
            assert isinstance(mv, dict) and len(mv) > 0,                 f"model_versions should be a non-empty dict: {mv}"