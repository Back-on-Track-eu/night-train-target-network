...
distances, the timetable and the energy all follow from that, and every cost
downstream depends on this chain.
...
Gauge is respected rather than assumed away. Standard, Iberian, Russian and
...
Routing runs per consecutive stop pair and is cached. Stitching pairs gives
the same path as routing the whole list at once, because intermediate stops
are hard constraints either way, and it means a route sharing a leg with one
already computed is free.

## From a path to a timetable

Raw driving time from the routing engine assumes a train that cruises at
constant speed and never stops. Two corrections turn it into a schedule.
...
See [terrain and buffers](/methodology/route-context).
...
A night train is defined by when it runs, not only where. The tool builds an
outbound service, mirrors it into a return, and will stretch the schedule to
keep the journey inside the night window rather than arriving at an hour
nobody wants.

Time of day therefore drives cost as well as convenience: the share of each
country run falling inside that country's night tariff window sets what
fraction of the [track access charge](/cost/tac) and the
...
infrastructure manager would grant the slot. A route the tool plans is
technically coherent, not allocated.
