...
|---|---|---|---|---|
| Route & timetable builder | `0.9.40` | Route and timetable builder: turns a list of stops, a train composition, and a few mode selections into a complete route — trip pairs, travel and stopping times with schedule buffers, and a mirrored outbound/return night schedule. | [`model.py`](../backend/models/route/model.py) | [README.md](../backend/models/README.md) |
...
| Demand model | `0.1.0` | Manual demand model: a potential demand per year is an input, split into five traveller groups that each book only the classes they list — a fixed allocation rule seats them round by round, so one demand reads as a utilisation on every composition, and demand no listed class can seat is not served. Sales spread over the sellable OD pairs by a boarding × alighting stop-weight matrix, and every passenger is attributed by journey length to the plane, the car or an induced trip for the climate figures. Fares are a flat two-part tariff per class. | [`model.py`](../backend/models/demand/model.py) | [README.md](../backend/models/demand/README.md) |
...
|---|---|---|---|---|
| Input | `n_places_sold,od` | Places sold per connection and year | places/year | set by the tool user |
| Input | `s_class` | Additional-services revenue per passenger of that class, overridable per proposal | €/passenger | standard value [`SERVICES_EUR_PER_PAX_BY_CLASS`](#s-demand-services_eur_per_pax_by_class) |
...
|---|---|---|---|---|
| Input | `n_places_sold,od` | Places sold per connection and year | places/year | set by the tool user |
| Input | `c_cat` | Net catering contribution per passenger of that class, overridable per proposal | €/passenger | standard value [`CATERING_EUR_PER_PAX_BY_CLASS`](#s-demand-catering_eur_per_pax_by_class) |
