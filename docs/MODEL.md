...
|---|---|---|---|---|
| Route & timetable builder | `0.9.22` | Route and timetable builder: turns a list of stops, a train composition, and a few mode selections into a complete route — trip pairs, travel and stopping times with schedule buffers, and a mirrored outbound/return night schedule. | [`model.py`](../backend/models/route/model.py) | [README.md](../backend/models/README.md) |
...
| Cost & revenue evaluation | `0.9.19` | Cost and revenue evaluation: computes the operator's fixed and variable costs, the charges paid to infrastructure companies, and the ticket revenue of a route, then aggregates the result into views per route, trip pair, country, connection, route section, and stop. | [`model.py`](../backend/models/evaluation/model.py) | [README.md](../backend/models/evaluation/README.md) |
...
| Infrastructure parameter model | `0.9.2` | Infrastructure parameter model: per-country track access charges, station charges, electricity prices, terrain, schedule buffers, and minimum stopping times, with EU-average fallbacks — plus the catalog of possible night train stops. | [`model.py`](../backend/models/infrastructure/model.py) | [STOP_CLASSIFICATION.md](../backend/models/infrastructure/STOP_CLASSIFICATION.md) |
...
| <a id="p-input_params-country_relations-routing_status"></a>`routing_status` | Why this pair does or does not carry a rail distance: routed, prefiltered (too far apart to be worth routing), no_connection (no rail path exists), or snap_failed (a reference station could not be placed on the network). | — | — |
